window.fichePreparee={
  "nom": "Tyrant Demon King",
  "designation": "\"The Ruler\"",
  "titre": "Tyrant Demon King",
  "idJeu": "1936501",
  "attribut": {
    "nom": "@attribute.5",
    "icone": "assets/commun/icon_filter_attribute_05__8b5613471712850c.png",
    "couleur": "#bf7ee9",
    "detail": "Identifiant attribut DB_skin_base"
  },
  "race": {
    "nom": [
      "@race.1"
    ],
    "icone": [
      "assets/commun/icon_filter_kind_01__b2dfe4e53bdd1bf4.png"
    ]
  },
  "caracteristiques": [],
  "portrait": "assets/1936501/images/icon_hero_demon_king_human_0002_s__cf96ed239c22f52c.png",
  "rareteParDefaut": "UR",
  "presentation": "\"The Ruler\" Tyrant Demon King has a skill which can deal greater damage the higher his Attack.",
  "commandement": null,
  "talent": null,
  "grace": null,
  "union": null,
  "raretés": [
    {
      "id": "SSR",
      "nom": "SSR",
      "icone": "assets/commun/icon_filter_grade_SSR__ed30b11d8c06558d.png",
      "cadre": "assets/1936501/images/icon_frame_small_ssr__3b1a07b8b1c4adb5.png",
      "fond": "assets/commun/icon_hero_basic_s_5__d57710ef4255cc36.png",
      "coin": "assets/commun/common_level_frame_rank_ssr.png",
      "couleur": "#d9b569",
      "image": "assets/1936501/images/hero_illust_demon_king_human_1936501__b6573cde034f2bba.png",
      "legendeImage": "demon_king_human_0001",
      "niveau": 50,
      "statistiques": {
        "principales": [
          {
            "nom": "@stat.attack",
            "valeur": 550.0
          },
          {
            "nom": "@stat.defense",
            "valeur": 380.0
          },
          {
            "nom": "@stat.hp",
            "valeur": 7200.0
          },
          {
            "nom": "@stat.combatClass",
            "valeur": 4654
          }
        ],
        "secondaires": [
          {
            "nom": "@stat.critChance",
            "valeur": 100.0,
            "unite": "%"
          },
          {
            "nom": "@stat.critDamage",
            "valeur": 200.0,
            "unite": "%"
          },
          {
            "nom": "@stat.resistance",
            "valeur": null,
            "unite": "%"
          },
          {
            "nom": "@stat.critResistance",
            "valeur": 80.0,
            "unite": "%"
          },
          {
            "nom": "@stat.critDefense",
            "valeur": 75.0,
            "unite": "%"
          },
          {
            "nom": "@stat.pierceRate",
            "valeur": 100.0,
            "unite": "%"
          },
          {
            "nom": "@stat.regenerationRate",
            "valeur": 5.0,
            "unite": "%"
          },
          {
            "nom": "@stat.recoveryRate",
            "valeur": 110.0,
            "unite": "%"
          },
          {
            "nom": "@stat.lifesteal",
            "valeur": 10.0,
            "unite": "%"
          }
        ]
      },
      "competences": [
        {
          "id": "attaque-a",
          "nom": "\"Quietus\"",
          "image": "assets/1936501/images/icon_demon_king_human_ssr_type01_skill_01__80c2a61c71c34e40.png",
          "type": "@skill.attack",
          "cible": "@target.oneEnemy",
          "rangs": [
            {
              "rang": 1,
              "multiplicateur": 200.0,
              "description": "Inflicts [00D5FF]Eradication[-] damage equal to 200 % of Attack on @target.oneEnemy.\\n\\n[00D5FF]※Eradication: [FFAE00]+[-]60 % Attack when using a skill.\\n[FFAE00]2x[-] Crit Chance Increase.[-]",
              "idJeu": 1002001,
              "cadre": "assets/skill-frames/rank-1.png",
              "iconeType": "assets/skill-frames/atk.png"
            },
            {
              "rang": 2,
              "multiplicateur": 300.0,
              "description": "Inflicts [00D5FF]Eradication[-] damage equal to 300 % of Attack on @target.oneEnemy.\\n\\n[00D5FF]※Eradication: [FFAE00]+[-]60 % Attack when using a skill.\\n[FFAE00]2x[-] Crit Chance Increase.[-]",
              "idJeu": 1002002,
              "cadre": "assets/skill-frames/rank-2.png",
              "iconeType": "assets/skill-frames/atk.png"
            },
            {
              "rang": 3,
              "multiplicateur": 500.0,
              "description": "Inflicts [00D5FF]Eradication[-] damage equal to 500 % of Attack on @target.oneEnemy.\\n\\n[00D5FF]※Eradication: [FFAE00]+[-]60 % Attack when using a skill.\\n[FFAE00]2x[-] Crit Chance Increase.[-]",
              "idJeu": 1002003,
              "cadre": "assets/skill-frames/rank-3.png",
              "iconeType": "assets/skill-frames/atk.png"
            }
          ]
        },
        {
          "id": "attaque-b",
          "nom": "\"Sword of Imprisonment\"",
          "image": "assets/1936501/images/icon_demon_king_human_ssr_type01_skill_02__cada595c98c97e34.png",
          "type": "@skill.attack",
          "cible": "@target.allEnemies",
          "rangs": [
            {
              "rang": 1,
              "multiplicateur": 140.0,
              "description": "Inflicts damage equal to 140 % of Attack on @target.allEnemies and applies [00D5FF]Mark of Death[-] for 1 tour(s).\\n\\n[00D5FF]※Mark of Death: Restricts Recovery-related stats.\\n[FFAE00]-[-]40 % Crit Resistance and Crit Defense.[-]",
              "idJeu": 1002011,
              "cadre": "assets/skill-frames/rank-1.png",
              "iconeType": "assets/skill-frames/debuff_atk_global.png"
            },
            {
              "rang": 2,
              "multiplicateur": 210.0,
              "description": "Inflicts damage equal to 210 % of Attack on @target.allEnemies and applies [00D5FF]Mark of Death[-] for 1 tour(s).\\n\\n[00D5FF]※Mark of Death: Restricts Recovery-related stats.\\n[FFAE00]-[-]40 % Crit Resistance and Crit Defense.[-]",
              "idJeu": 1002012,
              "cadre": "assets/skill-frames/rank-2.png",
              "iconeType": "assets/skill-frames/debuff_atk_global.png"
            },
            {
              "rang": 3,
              "multiplicateur": 350.0,
              "description": "Inflicts damage equal to 350 % of Attack on @target.allEnemies and applies [00D5FF]Mark of Death[-] for 2 tour(s).\\n\\n[00D5FF]※Mark of Death: Restricts Recovery-related stats.\\n[FFAE00]-[-]40 % Crit Resistance and Crit Defense.[-]",
              "idJeu": 1002013,
              "cadre": "assets/skill-frames/rank-3.png",
              "iconeType": "assets/skill-frames/debuff_atk_global.png"
            }
          ]
        }
      ],
      "ultime": {
        "nom": "\"Incursion\"",
        "image": "assets/1936501/images/icon_demon_king_human_ssr_type01_special_01__7b8ba6b09e95fbf9.png",
        "cible": "@target.allEnemies",
        "niveaux": [
          {
            "niveau": 1,
            "multiplicateur": 500.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]10%[-] for [FFAE00]2 turns[-], inflicts damage equal to 500 % of Attack and applies a [FFAE00]Lv. 1[-] [00D5FF]Blackout Effect[-] on @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 1[-] Blackout Effect: Disables Buffs and Debuffs.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-1.png"
          },
          {
            "niveau": 2,
            "multiplicateur": 550.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]15%[-] for [FFAE00]2 turns[-], inflicts damage equal to 550 % of Attack and applies a [FFAE00]Lv. 1[-] [00D5FF]Blackout Effect[-] on @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 1[-] Blackout Effect: Disables Buffs and Debuffs.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-2.png"
          },
          {
            "niveau": 3,
            "multiplicateur": 600.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]20%[-] for [FFAE00]2 turns[-], inflicts damage equal to 600 % of Attack and applies a [FFAE00]Lv. 2[-] [00D5FF]Blackout Effect[-] to @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 2[-] Blackout Effect: Disables Buffs, Debuffs, and Attack Skills.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-3.png"
          },
          {
            "niveau": 4,
            "multiplicateur": 650.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]30%[-] for [FFAE00]2 turns[-], inflicts damage equal to 650 % of Attack and applies a [FFAE00]Lv. 2[-] [00D5FF]Blackout Effect[-] to @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 2[-] Blackout Effect: Disables Buffs, Debuffs, and Attack Skills.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-4.png"
          },
          {
            "niveau": 5,
            "multiplicateur": 700.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]40%[-] for [FFAE00]2 turns[-], inflicts damage equal to 700 % of Attack and applies a [FFAE00]Lv. 3[-] [00D5FF]Blackout Effect[-] on @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 3[-] Blackout Effect: Disables all skills except Ultimate Moves.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-5.png"
          },
          {
            "niveau": 6,
            "multiplicateur": 750.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]50%[-] for [FFAE00]2 turns[-], inflicts damage equal to 750 % of Attack and applies a [FFAE00]Lv. 4[-] [00D5FF]Blackout Effect[-] on @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 4[-] Blackout Effect: Disables all skills including Ultimate Moves.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-6.png"
          }
        ]
      },
      "passif": {
        "nom": "Ruler's Reign",
        "image": "assets/1936501/images/icon_exclusive_skill_1936501__90b9edc31732db52.png",
        "description": "At the start of the battle, applies [00D5FF]Ruler's Reign[-] to self for [FFAE00]2 turns[-] and [00D5FF]Retrocession[-] to [00D5FF]Demon allies[-] for [FFAE00]2 turns[-]. For [FFAE00]each[-] [00D5FF]Retrocession[-] effect removed at the beginning of the allies' turn, increases the hero's Attack-related stats by [FFAE00]18%[-]. In addition, if the hero is [00D5FF]alive on the battlefield[-], all [00D5FF]Commandments are sealed[-], and for [FFAE00]each[-] dead [00D5FF]ally with a Commandment[-], the hero's Attack increases by [FFAE00]30%[-].\\n\\n[00D5FF]※Ruler's Reign: The hero's damage taken from skills during the enemy's turn does not exceed [FFAE00]50%[-] of Max HP.\\nOnce removed, all Debuffs are removed from the hero along with it, then all of the hero's stats are increased by [FFAE00]20%[-] and skills are ranked up.\\n[00D5FF]※Retrocession: [FFAE00]-20%[-] damage taken.[-]",
        "niveauxUltime": [
          {
            "niveau": 1,
            "description": "At the start of the battle, applies [00D5FF]Ruler's Reign[-] to self for [FFAE00]2 turns[-] and [00D5FF]Retrocession[-] to [00D5FF]Demon allies[-] for [FFAE00]2 turns[-]. For [FFAE00]each[-] [00D5FF]Retrocession[-] effect removed at the beginning of the allies' turn, increases the hero's Attack-related stats by [FFAE00]18%[-]. In addition, if the hero is [00D5FF]alive on the battlefield[-], all [00D5FF]Commandments are sealed[-], and for [FFAE00]each[-] dead [00D5FF]ally with a Commandment[-], the hero's Attack increases by [FFAE00]30%[-].\\n\\n[00D5FF]※Ruler's Reign: The hero's damage taken from skills during the enemy's turn does not exceed [FFAE00]50%[-] of Max HP.\\nOnce removed, all Debuffs are removed from the hero along with it, then all of the hero's stats are increased by [FFAE00]20%[-] and skills are ranked up.\\n[00D5FF]※Retrocession: [FFAE00]-20%[-] damage taken.[-]"
          }
        ]
      },
      "idForme": 315501,
      "portrait": "assets/1936501/images/icon_hero_demon_king_human_0001_s__90b917eb09b4786e.png",
      "portraitVertical": "assets/1936501/images/icon_skin_demon_king_human_0001_l__78cbc2b42d0799ab.png"
    },
    {
      "id": "UR",
      "nom": "UR",
      "icone": "assets/commun/icon_filter_grade_UR__e991ffc663c6f527.png",
      "cadre": "assets/1936501/images/icon_frame_small_ur__32674fdb5a494112.png",
      "fond": "assets/commun/icon_hero_basic_s_6__9b21faac625aca80.png",
      "coin": "assets/commun/common_level_frame_rank_ur.png",
      "couleur": "#c6a5fa",
      "image": "assets/1936501/images/hero_illust_demon_king_human_1936501__b6573cde034f2bba.png",
      "legendeImage": "demon_king_human_0002",
      "niveau": 60,
      "statistiques": {
        "principales": [
          {
            "nom": "@stat.attack",
            "valeur": 870.0
          },
          {
            "nom": "@stat.defense",
            "valeur": 450.0
          },
          {
            "nom": "@stat.hp",
            "valeur": 8800.0
          },
          {
            "nom": "@stat.combatClass",
            "valeur": 5350
          }
        ],
        "secondaires": [
          {
            "nom": "@stat.critChance",
            "valeur": 100.0,
            "unite": "%"
          },
          {
            "nom": "@stat.critDamage",
            "valeur": 200.0,
            "unite": "%"
          },
          {
            "nom": "@stat.resistance",
            "valeur": null,
            "unite": "%"
          },
          {
            "nom": "@stat.critResistance",
            "valeur": 80.0,
            "unite": "%"
          },
          {
            "nom": "@stat.critDefense",
            "valeur": 75.0,
            "unite": "%"
          },
          {
            "nom": "@stat.pierceRate",
            "valeur": 100.0,
            "unite": "%"
          },
          {
            "nom": "@stat.regenerationRate",
            "valeur": 5.0,
            "unite": "%"
          },
          {
            "nom": "@stat.recoveryRate",
            "valeur": 110.0,
            "unite": "%"
          },
          {
            "nom": "@stat.lifesteal",
            "valeur": 10.0,
            "unite": "%"
          }
        ]
      },
      "competences": [
        {
          "id": "attaque-a",
          "nom": "\"Quietus\"",
          "image": "assets/1936501/images/icon_demon_king_human_ssr_type01_skill_01__80c2a61c71c34e40.png",
          "type": "@skill.attack",
          "cible": "@target.oneEnemy",
          "rangs": [
            {
              "rang": 1,
              "multiplicateur": 200.0,
              "description": "Inflicts [00D5FF]Eradication[-] damage equal to 200 % of Attack on @target.oneEnemy.\\n\\n[00D5FF]※Eradication: [FFAE00]+[-]60 % Attack when using a skill.\\n[FFAE00]2x[-] Crit Chance Increase.[-]",
              "idJeu": 1002001,
              "cadre": "assets/skill-frames/rank-1.png",
              "iconeType": "assets/skill-frames/atk.png"
            },
            {
              "rang": 2,
              "multiplicateur": 300.0,
              "description": "Inflicts [00D5FF]Eradication[-] damage equal to 300 % of Attack on @target.oneEnemy.\\n\\n[00D5FF]※Eradication: [FFAE00]+[-]60 % Attack when using a skill.\\n[FFAE00]2x[-] Crit Chance Increase.[-]",
              "idJeu": 1002002,
              "cadre": "assets/skill-frames/rank-2.png",
              "iconeType": "assets/skill-frames/atk.png"
            },
            {
              "rang": 3,
              "multiplicateur": 500.0,
              "description": "Inflicts [00D5FF]Eradication[-] damage equal to 500 % of Attack on @target.oneEnemy.\\n\\n[00D5FF]※Eradication: [FFAE00]+[-]60 % Attack when using a skill.\\n[FFAE00]2x[-] Crit Chance Increase.[-]",
              "idJeu": 1002003,
              "cadre": "assets/skill-frames/rank-3.png",
              "iconeType": "assets/skill-frames/atk.png"
            }
          ]
        },
        {
          "id": "attaque-b",
          "nom": "\"Sword of Imprisonment\"",
          "image": "assets/1936501/images/icon_demon_king_human_ssr_type01_skill_02__cada595c98c97e34.png",
          "type": "@skill.attack",
          "cible": "@target.allEnemies",
          "rangs": [
            {
              "rang": 1,
              "multiplicateur": 140.0,
              "description": "Inflicts damage equal to 140 % of Attack on @target.allEnemies and applies [00D5FF]Mark of Death[-] for 1 tour(s).\\n\\n[00D5FF]※Mark of Death: Restricts Recovery-related stats.\\n[FFAE00]-[-]40 % Crit Resistance and Crit Defense.[-]",
              "idJeu": 1002011,
              "cadre": "assets/skill-frames/rank-1.png",
              "iconeType": "assets/skill-frames/debuff_atk_global.png"
            },
            {
              "rang": 2,
              "multiplicateur": 210.0,
              "description": "Inflicts damage equal to 210 % of Attack on @target.allEnemies and applies [00D5FF]Mark of Death[-] for 1 tour(s).\\n\\n[00D5FF]※Mark of Death: Restricts Recovery-related stats.\\n[FFAE00]-[-]40 % Crit Resistance and Crit Defense.[-]",
              "idJeu": 1002012,
              "cadre": "assets/skill-frames/rank-2.png",
              "iconeType": "assets/skill-frames/debuff_atk_global.png"
            },
            {
              "rang": 3,
              "multiplicateur": 350.0,
              "description": "Inflicts damage equal to 350 % of Attack on @target.allEnemies and applies [00D5FF]Mark of Death[-] for 2 tour(s).\\n\\n[00D5FF]※Mark of Death: Restricts Recovery-related stats.\\n[FFAE00]-[-]40 % Crit Resistance and Crit Defense.[-]",
              "idJeu": 1002013,
              "cadre": "assets/skill-frames/rank-3.png",
              "iconeType": "assets/skill-frames/debuff_atk_global.png"
            }
          ]
        }
      ],
      "ultime": {
        "nom": "\"Incursion\"",
        "image": "assets/1936501/images/icon_demon_king_human_ssr_type01_special_01__7b8ba6b09e95fbf9.png",
        "cible": "@target.allEnemies",
        "niveaux": [
          {
            "niveau": 1,
            "multiplicateur": 500.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]10%[-] for [FFAE00]2 turns[-], inflicts damage equal to 500 % of Attack and applies a [FFAE00]Lv. 1[-] [00D5FF]Blackout Effect[-] on @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 1[-] Blackout Effect: Disables Buffs and Debuffs.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-1.png"
          },
          {
            "niveau": 2,
            "multiplicateur": 550.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]15%[-] for [FFAE00]2 turns[-], inflicts damage equal to 550 % of Attack and applies a [FFAE00]Lv. 1[-] [00D5FF]Blackout Effect[-] on @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 1[-] Blackout Effect: Disables Buffs and Debuffs.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-2.png"
          },
          {
            "niveau": 3,
            "multiplicateur": 600.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]20%[-] for [FFAE00]2 turns[-], inflicts damage equal to 600 % of Attack and applies a [FFAE00]Lv. 2[-] [00D5FF]Blackout Effect[-] to @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 2[-] Blackout Effect: Disables Buffs, Debuffs, and Attack Skills.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-3.png"
          },
          {
            "niveau": 4,
            "multiplicateur": 650.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]30%[-] for [FFAE00]2 turns[-], inflicts damage equal to 650 % of Attack and applies a [FFAE00]Lv. 2[-] [00D5FF]Blackout Effect[-] to @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 2[-] Blackout Effect: Disables Buffs, Debuffs, and Attack Skills.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-4.png"
          },
          {
            "niveau": 5,
            "multiplicateur": 700.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]40%[-] for [FFAE00]2 turns[-], inflicts damage equal to 700 % of Attack and applies a [FFAE00]Lv. 3[-] [00D5FF]Blackout Effect[-] on @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 3[-] Blackout Effect: Disables all skills except Ultimate Moves.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-5.png"
          },
          {
            "niveau": 6,
            "multiplicateur": 750.0,
            "description": "Decreases all enemies' Defense-related stats by [FFAE00]50%[-] for [FFAE00]2 turns[-], inflicts damage equal to 750 % of Attack and applies a [FFAE00]Lv. 4[-] [00D5FF]Blackout Effect[-] on @target.allEnemies for [FFAE00]2 turns[-].\\n\\n[00D5FF]※[FFAE00]Lv. 4[-] Blackout Effect: Disables all skills including Ultimate Moves.[-]",
            "cadre": "assets/skill-frames/ultimate-normal-6.png"
          }
        ]
      },
      "passif": {
        "nom": "Ruler's Reign",
        "image": "assets/1936501/images/icon_exclusive_skill_1936501__90b9edc31732db52.png",
        "description": "At the start of the battle, applies [00D5FF]Ruler's Reign[-] to self for [FFAE00]2 turns[-] and [00D5FF]Retrocession[-] to [00D5FF]Demon allies[-] for [FFAE00]2 turns[-]. For [FFAE00]each[-] [00D5FF]Retrocession[-] effect removed at the beginning of the allies' turn, increases the hero's Attack-related stats by [FFAE00]18%[-]. In addition, if the hero is [00D5FF]alive on the battlefield[-], all [00D5FF]Commandments are sealed[-], and for [FFAE00]each[-] dead [00D5FF]ally with a Commandment[-], the hero's Attack increases by [FFAE00]30%[-].\\n\\n[00D5FF]※Ruler's Reign: The hero's damage taken from skills during the enemy's turn does not exceed [FFAE00]50%[-] of Max HP.\\nOnce removed, all Debuffs are removed from the hero along with it, then all of the hero's stats are increased by [FFAE00]20%[-] and skills are ranked up.\\n[00D5FF]※Retrocession: [FFAE00]-20%[-] damage taken.[-]",
        "niveauxUltime": [
          {
            "niveau": 1,
            "description": "At the start of the battle, applies [00D5FF]Ruler's Reign[-] to self for [FFAE00]2 turns[-] and [00D5FF]Retrocession[-] to [00D5FF]Demon allies[-] for [FFAE00]2 turns[-]. For [FFAE00]each[-] [00D5FF]Retrocession[-] effect removed at the beginning of the allies' turn, increases the hero's Attack-related stats by [FFAE00]18%[-]. In addition, if the hero is [00D5FF]alive on the battlefield[-], all [00D5FF]Commandments are sealed[-], and for [FFAE00]each[-] dead [00D5FF]ally with a Commandment[-], the hero's Attack increases by [FFAE00]30%[-].\\n\\n[00D5FF]※Ruler's Reign: The hero's damage taken from skills during the enemy's turn does not exceed [FFAE00]50%[-] of Max HP.\\nOnce removed, all Debuffs are removed from the hero along with it, then all of the hero's stats are increased by [FFAE00]20%[-] and skills are ranked up.\\n[00D5FF]※Retrocession: [FFAE00]-20%[-] damage taken.[-]"
          }
        ]
      },
      "idForme": 315502,
      "portrait": "assets/1936501/images/icon_hero_demon_king_human_0002_s__cf96ed239c22f52c.png",
      "portraitVertical": "assets/1936501/images/icon_skin_demon_king_human_0002_l__020554852457910a.png"
    }
  ],
  "costumes": [
    {
      "id": "3315501",
      "categorie": "Skin",
      "nom": "Cold-hearted Father",
      "image": null,
      "rarete": ""
    },
    {
      "id": "3315550",
      "categorie": "Skin",
      "nom": "Black Beast",
      "image": "assets/1936501/images/icon_skin_demon_king_human_0003_l__e36c2076062c316f.png",
      "rarete": ""
    },
    {
      "id": "3315551",
      "categorie": "Skin",
      "nom": "Merciless Conqueror",
      "image": "assets/1936501/images/icon_skin_demon_king_human_0004_l__81a73909bb440de3.png",
      "rarete": ""
    },
    {
      "id": "3315552",
      "categorie": "Skin",
      "nom": "Emperor",
      "image": "assets/1936501/images/icon_skin_demon_king_human_0005_l__5692dd299d0851cb.png",
      "rarete": ""
    },
    {
      "id": "3315554",
      "categorie": "Skin",
      "nom": "Devourer of the Abyss",
      "image": "assets/1936501/images/icon_skin_demon_king_human_0007_l__48fc9fc55b369b19.png",
      "rarete": ""
    },
    {
      "id": "3315553",
      "categorie": "Skin",
      "nom": "Fencing Master",
      "image": "assets/1936501/images/icon_skin_demon_king_human_0006_l__85535d9ea6684b51.png",
      "rarete": ""
    },
    {
      "id": "3315502",
      "categorie": "Skin",
      "nom": "Absolute Evil",
      "image": "assets/1936501/images/icon_skin_demon_king_human_0002_l__020554852457910a.png",
      "rarete": ""
    },
    {
      "id": "875201",
      "categorie": "Weapon",
      "nom": "Demon King's Sword",
      "image": "assets/1936501/images/icon_weapon_demon_king_human_0001_l__61980110edd6b27d.png",
      "rarete": ""
    },
    {
      "id": "875202",
      "categorie": "Weapon",
      "nom": "Beast's Fang",
      "image": "assets/1936501/images/icon_weapon_demon_king_human_0003_l__2a7084762be2aa84.png",
      "rarete": ""
    },
    {
      "id": "875203",
      "categorie": "Weapon",
      "nom": "Conqueror's Blade",
      "image": "assets/1936501/images/icon_weapon_demon_king_human_0004_l__b5045b8c6b4e709b.png",
      "rarete": ""
    },
    {
      "id": "875204",
      "categorie": "Weapon",
      "nom": "Treasured Sword",
      "image": "assets/1936501/images/icon_weapon_demon_king_human_0005_l__2235b478e2551551.png",
      "rarete": ""
    },
    {
      "id": "875206",
      "categorie": "Weapon",
      "nom": "Nidhoggr's Venomous Fang",
      "image": "assets/1936501/images/icon_weapon_demon_king_human_0007_l__96b20f7813c6776b.png",
      "rarete": ""
    },
    {
      "id": "875205",
      "categorie": "Weapon",
      "nom": "Competition Sabre",
      "image": "assets/1936501/images/icon_weapon_demon_king_human_0006_l__502a11bd0f8ef421.png",
      "rarete": ""
    },
    {
      "id": "875207",
      "categorie": "Weapon",
      "nom": "Fafnir's Cursed Sword",
      "image": "assets/1936501/images/icon_weapon_demon_king_human_0001_l__61980110edd6b27d.png",
      "rarete": ""
    },
    {
      "id": "4003901",
      "categorie": "Head",
      "nom": "Basic Hairstyle",
      "image": "assets/1936501/images/icon_head_demon_king_human_0001_l__058339727bec3721.png",
      "rarete": ""
    },
    {
      "id": "4003902",
      "categorie": "Head",
      "nom": "Wild Hair",
      "image": "assets/1936501/images/icon_head_demon_king_human_0003_l__62733e2973e76fb2.png",
      "rarete": ""
    },
    {
      "id": "4003903",
      "categorie": "Head",
      "nom": "Laurel of the Conqueror",
      "image": "assets/1936501/images/icon_head_demon_king_human_0004_l__1c8ebdc78cb03151.png",
      "rarete": ""
    },
    {
      "id": "4003904",
      "categorie": "Head",
      "nom": "Imperial Crown",
      "image": "assets/1936501/images/icon_head_demon_king_human_0005_l__04cbb0f25d06668e.png",
      "rarete": ""
    },
    {
      "id": "4003907",
      "categorie": "Head",
      "nom": "Abyss Horn",
      "image": "assets/1936501/images/icon_head_demon_king_human_0007_l__a7c2957a410d764f.png",
      "rarete": ""
    },
    {
      "id": "4003906",
      "categorie": "Head",
      "nom": "Fencing Mask",
      "image": "assets/1936501/images/icon_head_demon_king_human_0006_l__dd9b7e3289748afc.png",
      "rarete": ""
    },
    {
      "id": "4003905",
      "categorie": "Head",
      "nom": "Zeldris Style",
      "image": "assets/1936501/images/icon_head_demon_king_human_0501_l__15a48c4483020728.png",
      "rarete": ""
    }
  ],
  "relique": {
    "nom": "Fafnir's Cursed Sword",
    "image": "assets/1936501/images/icon_weapon_demon_king_human_0551_s__5daaf0bb5b1d8c5f.png",
    "description": "Tyrant Demon King's Holy Relic (Weapon).\\n[DC143C]A sword containing the curse of Fafnir. It's a sword that lay hidden in Fafnir's treasure hoard, imbued with a terrible curse that prevents anyone from touching it.[-]",
    "passif": {
      "nom": "Fafnir's Cursed Sword",
      "image": "assets/commun/icon_setcostume_craft_001__d025bdbd539c2c8e.png",
      "description": "Increases the hero's Attack-related stats by [FFAE00]10%[-] for [FFAE00]each[-] [00D5FF]Retrocession[-] effect on allies.\\n\\nFor [FFAE00]each[-] [00D5FF]Retrocession[-] effect removed, the hero recovers [FFAE00]10%[-] of their Max HP.\\n\\nWhen a [00D5FF]Retrocession[-] is removed from the hero, the hero's damage taken decreases by [FFAE00]50%[-] for [FFAE00]2 turns[-]."
    },
    "bonus": [
      {
        "nom": "@stat.attack",
        "valeur": 460.0
      },
      {
        "nom": "@stat.defense",
        "valeur": 350.0
      },
      {
        "nom": "@stat.hp",
        "valeur": 3800.0
      }
    ],
    "materiaux": [
      {
        "nom": "Gold",
        "image": "assets/commun/icon_item_money_gold__f3bf90ba36454792.png",
        "quantite": 5000000
      },
      {
        "nom": "Source of the Cycle",
        "image": "assets/commun/icon_craft_costume_rare_material_05__19b8fbecda8796b5.png",
        "quantite": 15
      },
      {
        "nom": "Fruit of Yggdrasil",
        "image": "assets/commun/icon_craft_costume_material_09__a8fd9d110158c0cc.png",
        "quantite": 30
      },
      {
        "nom": "Black Magic Essence",
        "image": "assets/commun/icon_craft_costume_key_material_02__c45289b47672f7a7.png",
        "quantite": 80
      },
      {
        "nom": "Volundr's Tenacity",
        "image": "assets/commun/icon_craft_costume_material_12__931f89e710e00615.png",
        "quantite": 80
      }
    ],
    "provenance": "Ratatoskr"
  }
};
